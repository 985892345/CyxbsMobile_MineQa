 ...
 */
public class MZScaleInTransformer implements ViewPager2.PageTransformer {
    ...
    public void transformPage(@NonNull View view, float position) {
        ...
        if (currentPos <= -1.0f) {
            ...
            float distance = Math.abs(Math.abs(currentPos) - 0.5f) * 2;
            if (currentPos <= -0.5) {
                view.setTranslationX(translationX + distance);
